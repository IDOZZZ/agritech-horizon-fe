    ...
    <div className="self-stretch py-10 md:py-20 bg-white overflow-hidden flex-col justify-start items-center gap-6 flex">
        ...
        <div className="w-full px-4 md:px-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 justify-items-center">
          {categories.map((item, index) => (
            <div key={item.id} className="w-full max-w-sm h-[440px] p-2 bg-gradient-to-b from-black/20 to-black/20 bg-cover flex-col justify-end items-center gap-2.5 flex relative overflow-hidden rounded-sm">
                <img 
                    src={localImages[index % localImages.length]} // Use local images based on index
                    alt={item.name} 
                    className="absolute inset-0 z-[-1] rounded-sm object-cover w-full h-full"
                />
                <div className="w-full p-3 bg-black bg-opacity-30 rounded-sm flex-col justify-start items-start gap-2 flex z-10">
                    <div className="w-full flex-col justify-start items-start gap-1 flex">
                        <div className="self-stretch text-white text-xl md:text-2xl font-bold font-['Metropolis'] leading-tight">{item.name}</div>
                        <div className="self-stretch text-white text-sm md:text-base font-normal font-['Manrope'] leading-tight">{item.description}</div>
                    </div>
                    <div className="w-full flex flex-col sm:flex-row gap-3">
                        <Link href={`/courses?category=${item.id}`} data-color="Primary" data-kind="Primary" data-round="Squire" data-state="Default" className="w-full p-2.5 bg-[#0F5028] rounded-sm flex justify-center items-center">
